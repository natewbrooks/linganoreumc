...
function EventForm({ mode = 'create', initialData = null, setParentEventID = null }) {
		...
		if (mode === 'edit' && initialData) {
			...
			if (initialData.eventDateData) {
				const converted = initialData.eventDateData.map((dateEntry) => {
					const times =
						(dateEntry.times || []).length > 0
							? dateEntry.times.map((t) =>
									typeof t === 'string'
										? { startTime: t, endTime: '' }
				...
				setDateTimeData([{ date: '', times: [{ startTime: '', endTime: '' }] }]);
			}
	...
	const handleSubmit = async (e) => {
		...
		const hasAtLeastOneTime = dateTimeData.some(
			(d) => Array.isArray(d.times) && d.times.some((t) => (t?.startTime?.trim?.() || '') !== '')
		);
		...
		try {
			...
			const sortedDateTimeData = [...dateTimeData]
				...
				.map((dateObj) => ({
					...
					times: [...dateObj.times].sort((a, b) => {
						const timeA = new Date(`1970-01-01T${a.startTime || '00:00'}:00`).getTime();
						const timeB = new Date(`1970-01-01T${b.startTime || '00:00'}:00`).getTime();
						return timeA - timeB;
					}),
				}));
			...
			for (const { date, times, isCancelled = false, eventDateID } of sortedDateTimeData) {
				...
				for (const { startTime, endTime } of times) {
					if (finalDateID && startTime && startTime.trim() !== '') {
						await createEventTime(finalDateID, startTime, endTime || null);
					} else {
						console.error('Invalid data for creating event time:', { finalDateID, startTime });
					}
				}
			}
