...
export default function SelectImageSingle({
	...
	folderFilter = `${process.env.NEXT_PUBLIC_API_BASE_URL}/media/images`,
}) {
	...
	const handleDeleteImage = async (filename) => {
		try {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/media/images/${filename}`,
				{
					method: 'DELETE',
				}
			);
		...
		<div className='w-full'>
			<SelectImageDropdown
				...
				initialSelectedImage={
					image.url?.split('/').pop()
						? `${process.env.NEXT_PUBLIC_API_BASE_URL}/media/images/${image.url.split('/').pop()}`
						: ''
				}
