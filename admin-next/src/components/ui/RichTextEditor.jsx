...
import { FaLink, FaAlignCenter, FaAlignLeft } from 'react-icons/fa';
...
const RichTextEditor = ({ value, onChange, title }) => {
	...
	// Function to handle blockquote for selected content only
	const toggleBlockquoteForSelection = () => {
		if (!editor) return;
		...
		doc.nodesBetween(from, to, (node, pos) => {
			if (node.type.name === 'blockquote') {
				...
				// Lift the blockquote node directly within selection
				tr.lift(tr.doc.resolve(pos), tr.doc.resolve(pos + node.nodeSize));
			}
		});
	...
	return (
		<div className='h-full flex flex-col '>
			...
			<div className='flex flex-wrap space-x-4 text-bkg px-4 py-0.5 bg-red items-center'>
				...
				<button
					type='button'
					onClick={toggleBlockquoteForSelection}
					className={`cursor-pointer hover:opacity-50 px-2 rounded ${
				...
				<button
					...
					className={`cursor-pointer hover:opacity-50 px-2 py-0.5 rounded ${
						editor.isActive({ textAlign: 'left' }) ? 'bg-bkg text-red' : ''
					}`}
					title='Align Left'>
					<FaAlignLeft size={18} />
				</button>
				<button
					...
					className={`cursor-pointer hover:opacity-50 px-2 py-0.5 h-full rounded ${
