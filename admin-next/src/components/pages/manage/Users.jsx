...
function Users() {
	...
	useEffect(() => {
		if (user?.role === 'admin') {
			axios
				.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/users`, { withCredentials: true })
	...
	const handleCreateUser = async () => {
		try {
			...
			const updated = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/users`, {
				withCredentials: true,
			});
	...
	const handleDeleteUser = async (id) => {
		...
		try {
			await axios.delete(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/users/${id}`, {
				withCredentials: true,
			});
							...
							<div className='flex justify-end space-x-2'>
								...
								<button
									onClick={async () => {
										...
										try {
											await axios.put(
												`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/users/${editNameModal.id}`,
												{
